...
from models.tools.MultiHeadAttention import MultiHeadAttention
...
class ADGen(nn.Module):
    ...
    def __init__(self, input_dim, dim, style_dim, n_downsample, n_res, mlp_dim, activ='relu', pad_type='reflect'):
        ...
        # self.attention0 = MultiHeadAttention(64, 64, 64, 64, 2, 0.5)
        self.attention1 = MultiHeadAttention(128, 128, 128, 128, 2, 0.5)
        # self.attention2 = MultiHeadAttention(256, 256, 256, 256, 4, 0.5)
    ...
    def forward(self, img_A, img_B, sem_B):
        ...
        feature = {}
        # feature['att0'] = self.attention0(self.enc_content.content['content0'], self.enc_style.style['style0'], self.enc_style.style['style0'],
        #                                   valid_lens=None)
        feature['att1'] = self.attention1(self.enc_content.content['content1'], self.enc_style.style['style1'], self.enc_style.style['style1'],
                                          valid_lens=None)
        # feature['att2'] = self.attention2(self.enc_content.content['content2'], self.enc_style.style['style2'], self.enc_style.style['style2'],
        #                                   valid_lens=None)
        images_recon = self.dec(content, style, feature)
        return images_recon
...
class ContentEncoder(nn.Module):
    def __init__(self, layers=2, ngf=64, img_f=512, use_spect=False, use_coord=False):
        ...
        self.ngf = ngf
        self.img_f = img_f
        self.block0 = EncoderBlock(30, ngf, norm_layer, nonlinearity, use_spect, use_coord, downSampling=False)
        mult = 1
        ...
        self.model0 += [nn.Conv2d(128, 256, 4, 2, 1)]
        self.model0 = nn.Sequential(*self.model0)
        self.content = {}

    def forward(self, x):
        out = self.block0(x)
        # self.content['content0'] = out
        for i in range(self.layers - 1):
            model = getattr(self, 'encoder' + str(i))
            out = model(out)
        self.content['content1'] = out
        out = self.model0(out)
        # self.content['content2'] = out
        return out
...
class Decoder(nn.Module):
    ...
    def forward(self, x, style, feature):
        # fusion module
...
class EncoderBlock(nn.Module):
    def __init__(self, input_nc, output_nc, norm_layer=nn.BatchNorm2d, nonlinearity=nn.LeakyReLU(),
        ...
        if downSampling:
            ...
            conv2 = coord_conv(input_nc, output_nc, use_spect, use_coord, **kwargs_fine)
            if type(norm_layer) == type(None):
                self.model = nn.Sequential(nonlinearity, conv2, )
            else:
                self.model = nn.Sequential(norm_layer(input_nc), nonlinearity, conv2, )
