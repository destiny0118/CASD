...
class ADGen(nn.Module):
    ...
    def __init__(self, input_dim, dim, style_dim, n_downsample, n_res, mlp_dim, activ='relu', pad_type='reflect'):
        ...
        self.enc_style = VggStyleEncoder(3, input_dim, dim, int(style_dim / self.SP_input_nc), norm='none', activ=activ,
                                         pad_type=pad_type)
        self.enc_style2 = VggStyleEncoder(3, input_dim, dim, int(style_dim / self.SP_input_nc), norm='none', activ=activ,
                                         pad_type=pad_type)
...
class MyDecoder(nn.Module):
    def __init__(self, n_upsample, dim, output_dim, activ='relu', pad_type='zero'):
        ...
        norm_layer = get_norm_layer(norm_type='instance')
        nonlinearity = get_nonlinearity_layer(activation_type='LeakyReLU')
        self.block0 = EncoderBlock(128, 64, norm_layer,
                                   nonlinearity, False, False,False)
        self.model2 = []
        self.model2 += [Conv2dBlock(64, output_dim, 7, 1, 3, norm='none', activation='tanh', pad_type=pad_type)]
...
class EncoderBlock(nn.Module):
    def __init__(self, input_nc, output_nc, norm_layer=nn.BatchNorm2d, nonlinearity=nn.LeakyReLU(),
        ...
        conv3=coord_conv(input_nc, output_nc, use_spect, use_coord, **kwargs_fine)

        if type(norm_layer) == type(None):
            if downSampling:
                self.model = nn.Sequential(nonlinearity, conv1, nonlinearity, conv2,)
            else:
                self.model = nn.Sequential(nonlinearity, conv3,)
        else:
            if downSampling:
                self.model = nn.Sequential(norm_layer(input_nc),  nonlinearity, conv1,
                                       norm_layer(output_nc), nonlinearity, conv2,)
            else:
                self.model = nn.Sequential(norm_layer(input_nc), nonlinearity, conv3,)
